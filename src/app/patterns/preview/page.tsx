...
export default function PatternsPreviewPage() {
  ...
  const symptomPhasePatterns = analyzeTaggedPatternsAllPhases(completedCycles, symptomLogs)
    ...
    .map((p) => ({
      ...
      sentence: `You've logged ${SYMPTOM_LABEL_BY_KEY[p.key]?.toLowerCase() ?? p.key} during the ${phasePhrase(p.phase)} in ${p.occurrences} of your last ${p.eligibleCycles} cycles.`,
    }));
  ...
  const moodPatterns = analyzeTaggedPatternsAllPhases(completedCycles, moodLogs)
    ...
    .map((p) => ({
      ...
      occurrences: p.occurrences,
      eligibleCycles: p.eligibleCycles,
      sentence: `You've reported feeling ${MOOD_LABEL_BY_KEY[p.key] ?? p.key} during the ${phasePhrase(p.phase)} in ${p.occurrences} of your last ${p.eligibleCycles} cycles.`,
    }));
  ...
  const cravingPatterns = analyzeTaggedPatternsAllPhases(
    ...
    occurrences: p.occurrences,
    eligibleCycles: p.eligibleCycles,
    sentence: `You've logged cravings during the ${phasePhrase(p.phase)} in ${p.occurrences} of your last ${p.eligibleCycles} cycles.`,
  }));
